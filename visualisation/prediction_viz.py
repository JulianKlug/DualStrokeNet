...
data_dir = '/Users/julian/temp/carvana_segmentation/carvana_data_combiLoss/model_prediction'
setting = 'train'
...
for i_slice, pred in enumerate(predictions):
    ...
    for channel in range(n_c):
        visual_add(np.squeeze(inputs[i_slice, channel].detach().numpy()), i_slice, i_col, gs, '')
        i_col += 1
    visual_add(np.squeeze(lesions[i_slice].detach().numpy()), i_slice, i_col, gs, '')
    visual_add(np.squeeze(pred.detach().numpy()), i_slice, i_col + 1, gs, '')
    all_loss = CombinedDiceEntropyLoss().forward(pred, lesions[i_slice]).item()
    print(str(all_loss))
    ...
    dice = dice_score(hard_prediction, np.squeeze(lesions[i_slice])).item()
    print(str(dice))
